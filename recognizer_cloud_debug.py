...
def load_logs():
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print("Fetched JSON from JSONBin:")
            print(json.dumps(data, indent=2))  # Printing the whole response for inspection
            return data.get("logs", [])  # Handling case where "logs" doesn't exist
        else:
